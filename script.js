document.addEventListener('DOMContentLoaded', () => {
 ...
 document.getElementById('export-pdf-btn-s').onclick = () => exportReportToPDF(`Informe_Muestra_${id}`); reportBody.innerHTML = 'Generando...'; reportModal.classList.add('visible'); const sample = state.samples[id]; const { info, events, data, maxRoR } = sample; const asistenteAvisos = analizarPerfilCompleto(sample); const devTime = (events.crack && events.end) ? events.end.time - events.crack.time : 0; const filteredData = data.filter(d => d.temp !== null && !d.adjustment); const totalTime = events.end ? events.end.time : (filteredData.length > 0 ? filteredData[filteredData.length - 1].time : 0); const dtr = totalTime > 0 && devTime > 0 ? (devTime/totalTime*100).toFixed(1) : '0.0'; const loss = (info.greenWeight && info.roastedWeight) ? ((info.greenWeight-info.roastedWeight)/info.greenWeight*100).toFixed(2) : '0.00'; let logTableHtml = '<div class="report-log-container"><table class="report-table report-log-table"><thead><tr><th>Tiempo</th><th>Temp. (°C)</th><th>RoR (°C/min)</th></tr></thead><tbody>'; for (let i = 0; i < filteredData.length; i++) { const curr = filteredData[i]; let ror = '-'; if (i > 0) { const prev = filteredData[i-1]; const timeDiffMinutes = (curr.time - prev.time) / 60; if(timeDiffMinutes > 0) { ror = ((curr.temp - prev.temp) / timeDiffMinutes).toFixed(1); } } logTableHtml += `<tr><td>${formatTime(curr.time)}</td><td>${curr.temp.toFixed(1)}</td><td>${ror}</td></tr>`; } logTableHtml += '</tbody></table></div>'; let asistenteHtml = ''; if (asistenteAvisos && asistenteAvisos.length > 0) { asistenteHtml = `<div class="report-asistente-section"><h4>Observaciones del Asistente</h4>`; asistenteAvisos.forEach(aviso => { asistenteHtml += `<div class="aviso aviso-${aviso.tipoMensaje}">${aviso.texto}</div>`; }); asistenteHtml += `</div>`; } else { asistenteHtml = `<div class="report-asistente-section"><h4>Observaciones del Asistente</h4><p>No se registraron desviaciones significativas del perfil ideal durante este tueste.</p></div>`; } const img = await generateChartImageForReport(id, data, sample.adjustments); let equipoHtml = '';
    if (sample.equipo && sample.equipo.rol) {
        equipoHtml = `<tr><td colspan="2" style="background-color: var(--bg-dark); text-align: center; color: var(--text-color);"><strong>EQUIPO: ${sample.equipo.rol ? s.equipo.rol.toUpperCase() : '-'}</strong></td></tr>
