document.addEventListener('DOMContentLoaded', () => {
    ...
    if (s.equipo && s.equipo.rol) {
        equipoHtml = `<tr><td colspan="2" style="background-color: var(--bg-dark); text-align: center; color: var(--text-color);"><strong>EQUIPO: ${s.equipo.rol ? s.equipo.rol.toUpperCase() : '-'}</strong></td></tr>
        <tr><td>Maestro Tostador</td><td>${s.equipo.maestroTostador || '-'}</td></tr>`;
        if (s.equipo.rol === 'Estudiante') {
            equipoHtml += `<tr><td>Tostador Adjunto</td><td>${s.equipo.tostadorAdjunto || '-'}</td></tr>
            <tr><td>Reporte de Datos</td><td>${s.equipo.reporteDatos || '-'}</td></tr>
            <tr><td>Registro Físico</td><td>${s.equipo.registroFisico || '-'}</td></tr>
            <tr><td>Registro Plataforma</td><td>${s.equipo.registroPlataforma || '-'}</td></tr>`;
        }
    }
